...
function timeFormatByKey(params, keys) {
    try {
        const copyParam = structuredClone(params);
        if (keys.length === 0)
            return copyParam;
...
exports.default = timeFormatByKey;
function createDateTime(dateObj) {
    ...
    return formatDateToISOString(new Date(year, month, day, hour, minute, second, millisecond));
}
